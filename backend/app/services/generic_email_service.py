...
def send_generic_form_email(
    ...
    for key, value in form_data.items():
        if value and key not in ['email', 'name', 'files']:
            ...
            form_rows_html += f"""
            <tr>
              <td style="padding:10px;border:1px solid #e5e7eb;font-weight:600;width:180px;
              background:#f8fafc;">{label}:</td>
              <td style="padding:10px;border:1px solid #e5e7eb;white-space:pre-line;">
              {value_str}</td>
            </tr>
            """
    ...
    admin_body_html = f"""
    <div style="font-family:Arial,sans-serif;max-width:700px;margin:auto;border:1px solid #e9e9e9;">
      <div style="background-color:#2c7a4b;padding:20px;text-align:center;">
        <h1 style="color:white;margin:0;">He Cares Foundation</h1>
        <p style="color:#c8f5da;margin:5px 0 0;">New Form Submission</p>
      </div>
      <div style="padding:24px;">
        <h2 style="color:#2c7a4b;margin-bottom:12px;">New Form Submission</h2>
        <p style="font-size:14px;color:#666;margin-bottom:16px;">
          <em>Submitted from: {form_source}</em>
        </p>
        <table style="width:100%;border-collapse:collapse;font-size:14px;margin-top:20px;">
          <tr style="background:#f8fafc;">
            ...
            <td style="padding:10px;border:1px solid #e5e7eb;">
              <b style="color:#2c7a4b;">{reference}</b>
            </td>
          </tr>
          <tr>
        ...
        <h3 style="color:#2c7a4b;margin-top:20px;">Form Details:</h3>
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          {form_rows_html}
          {f"<tr><td style='padding:10px;border:1px solid #e5e7eb;font-weight:600;background:#f8fafc;vertical-align:top;'>Attachments</td><td style='padding:10px;border:1px solid #e5e7eb;'>{attachments_html}</td></tr>" if attachments_html else ""}
        </table>
      </div>
...
"""
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;border:1px solid #e9e9e9;">
      <div style="background-color:#2c7a4b;padding:20px;text-align:center;">
        <h1 style="color:white;margin:0;">He Cares Foundation</h1>
        <p style="color:#c8f5da;margin:5px 0 0;">Submission Received</p>
      </div>
      <div style="padding:28px;">
        <p style="font-size:16px;">Dear <b>{user_name}</b>,</p>
        <p style="font-size:15px;">
          Thank you for reaching out to <b>HE CARES FOUNDATION</b> via our 
          <b>{form_source}</b> form. We have received your submission and will 
          respond as soon as possible.
        </p>
        <div style="background:#f8fafc;border-radius:7px;padding:20px;margin:20px 0;
        border:1px solid #e3e7ee;">
          <div style="font-size:15px;"><b>Your Reference Number:</b></div>
          <div style="font-size:22px;color:#2c7a4b;font-weight:700;margin-top:8px;">
            {reference}
          </div>
        </div>
        <p style="font-size:15px;">
...
Thank you for reaching out to HE CARES FOUNDATION via our {form_source} form. 
We have received your submission and will respond as soon as possible.
...
"""
    ...
    if not EMAIL_USER or not EMAIL_PASS:
        print(f"[MOCK EMAIL] Form submission notification for {form_source} - Ref: {reference}")
    ...
    try:
        ...
        print(f"Form submission emails sent - {form_source} - Reference: {reference}")
    except Exception as e:
